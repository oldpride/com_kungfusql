...
from
   (
      select
         onepoll.*,
         SUBSTRING_INDEX(SUBSTRING_INDEX(onepoll.answer_ids, ', ', numbers.n), ', ', -1) answer_id
      from
         (
            ...
            select * from #__advpoll_logs where poll_id = 'kungfusql_param1') onepoll
            on CHAR_LENGTH(onepoll.answer_ids) 
               -CHAR_LENGTH(REPLACE(onepoll.answer_ids, ', ', ''))>=(numbers.n-1)*2
         ) split_log,
         #__users,
         (
